# =============================================================================
# windows-sinks.r2 — Native r2 security sink labeler for Windows PE binaries
# =============================================================================
#
# Language: native r2 command language (no Python, no external tools)
#
# Sourced automatically by windows-x86.r2 and windows-x64.r2 profiles
# after analysis completes (aa/aaa must have run first).
#
# What this script does
# ─────────────────────
# 1. Defines a helper macro that, given an import name:
#      a) Seeks to the IAT entry address
#      b) Creates  f sink.NAME.iat @ <IAT_addr>
#      c) Finds the JMP wrapper stub (via axt) and creates  f sink.NAME @ <stub_addr>
#      d) Annotates the IAT entry with  CC "<CATEGORY>: <reason>"
# 2. For every known dangerous Windows API function, conditionally invokes
#    the macro using  ?l + ?ne  (run only when the function is actually imported).
# 3. Detects and labels PE entry points (WinMain, DllMain, TLS callbacks).
# 4. Prints a one-line summary.
#
# Conditional execution
# ─────────────────────
#   ?l `ii~NAME~[1]`      — sets $? = length of address string (0 if not imported)
#   ?ne .(sink NAME cat reason)  — runs only if $? != 0 (function is present)
#
# Wrapper stub resolution
# ───────────────────────
# For each IAT entry (e.g. recv at 0x40619c), PE binaries typically have a
# JMP thunk like:
#   sub.WS2_32.dll_recv @ 0x4025fc:  jmp dword [sym.imp.WS2_32.dll_recv]
#
# Callers use the thunk address, not the raw IAT address.
# The macro finds the thunk via:
#   f sink.NAME @ `axt $$~jmp~[1]`
#
# If no JMP thunk exists (function called via IAT directly), the flag
# falls back to the IAT address — the `.iat` suffix flag is always present.
#
# Usage
# ─────
#   Sourced automatically by the Windows profile.
#   Manual use:
#     aa
#     . ~/.local/share/radare2/scripts/windows-sinks.r2
#     f~sink    — list all labeled sinks
#     f~entry   — list labeled entry points
#     axt sink.recv    — find all recv callers
#     axt sink.strcpy  — find all strcpy callers
# =============================================================================

# ── Internal macro ────────────────────────────────────────────────────────────
# Usage: .(sink FUNCNAME CATEGORY REASON)
#   $0 = function name (e.g. recv, strcpy, WinExec)
#   $1 = category tag (e.g. NET, COPY, CMD)
#   $2 = brief reason (single word — no spaces inside macro args)
#
# Seek to IAT → flag IAT → flag JMP wrapper → annotate
(sink n c r; s `ii~$0~[1]`; f sink.$0.iat @ $$; f sink.$0 @ `axt $$~jmp~[1]`; CC "$1:$2" @ $$)

# ── CMD — Command and process execution ───────────────────────────────────────
# Highest severity: direct code execution from user-controlled input
...
# ── COPY — Unsafe string and buffer copies ────────────────────────────────────
# Most common memory corruption primitive: user-controlled size into fixed buffer
...
# ── CRYPT — Weak/risky cryptographic operations ───────────────────────────────
...
# ── REG — Registry write (persists attacker data) ─────────────────────────────
...
# ── AUTH — Authentication functions (check return value handling) ─────────────
...
# Label well-known PE entry points as entry.* flags so analysts can find
# WinMain, DllMain and TLS callbacks without manual inspection.
...
# ── TLS callbacks from PE data directory ──────────────────────────────────────
# iT lists TLS directory; extract callback addresses if present
